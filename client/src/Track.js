...
function Track() {
  ...
  const loadBlockchaindata = async () => {
    ...
    if (networkData) {
      ...
      const medStage = [];
      for (i = 0; i < medCtr; i++) {
        med[i + 1] = await supplychain.methods.WheatStock(i + 1).call();
    ...
    return (
      <div className="container-xl">
        ...
        <section className="row">
                    ...
                    <article className="col-3">
                        <h4><u>Materials Supplied by:</u></h4>
  ...
  const handlerSubmit = async (event) => {
    ...
    var ctr = await SupplyChain.methods.WheatCtr().call();
    if (!(ID > 0 && ID <= ctr)) alert("Invalid Wheat ID!!!");
    else {
